...
const DEFAULT: RotorInputs = {
  rotorVoltage: 220, statorVoltage: 380, statorCurrent: 20,
  turnsRatio: 1.0, frequency: 50, polePairs: 2, slots: 28,
  currentDensity: 4.5, connection: 'star', layers: 2,
  boreDiameter: 108, stackLength: 130, windingFactor: 0.955, fluxDensity: 0.65,
};
...
export default function RotorCalculator() {
  ...
  const handleCalculate = () => {
    try {
      ...
      setRes(calcRotor(inp)); setError(null);
    } catch {
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionPanel title="Configuration du Bobinage Rotor">
        <SelectField label="Couplage rotor" value={inp.connection}
      ...
      {res && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            <ResultCard title="Courant et Section" icon={<Cable className="w-4 h-4" />} color="teal"
              items={[
            ...
            <ResultCard title="Fil Normalisé" icon={<Cable className="w-4 h-4" />} color="cyan"
              items={[
            ...
            <ResultCard title="Bobinage Rotor" icon={<Layers className="w-4 h-4" />} color="amber"
              items={[
      ...
      <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-slate-500 space-y-1">
            ...
            <p>Pour un rotor à cage, la section des barres est : S_barre = I_barre / J avec I_barre = I_stator × (2m × N_stator × kw_s) / (Zr × kw_r). Densité barres cuivre : 5–7 A/mm², aluminium : 3–5 A/mm².</p>
          </div>
        </div>
      </div>
