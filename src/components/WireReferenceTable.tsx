  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((w, i) => (
                <tr
                  key={w.diameter}
                  className={`transition-colors hover:bg-cyan-50 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}
                >
      ...
      <p className="text-xs text-slate-400 text-center">
        {filtered.length} fil(s) affiché(s) sur {WIRE_TABLE.length} — Courants à 20°C, J ≈ 5 A/mm² (moteurs), J ≈ 4.4 A/mm² (général)
      </p>
    </div>
  );
}
